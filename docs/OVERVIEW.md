...
| **3 — Web filter** | Whole categories (ads/trackers, social media, adult content) | A DNS-over-HTTPS configuration profile points the entire Mac at a filtering resolver. Five filters are offered: AdGuard Standard, Control D Social, Mullvad Extended, CleanBrowsing Adult, CleanBrowsing Family. Filtering happens before a connection is attempted, below every app. Enforced by the profile + PF firewall anchor + profile self-healer. |
